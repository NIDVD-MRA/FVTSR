...
def process_file(file_path):
    ...
    df.insert(8, 'Label_Corr_Coeff', 0)
    df.insert(9, 'BiLSTM_Corr_Coeff', 0)
    df.insert(10, 'BiGRU_Corr_Coeff', 0)
    ...
    for idx, row in df.iterrows():
        ...
        print(row[13:])
        points = row.iloc[13:].values.astype(np.float64)  # 获取时间序列数据
        ...
        bigru_start_pos = int(row['Pred_Start_Pos_BiGRU']) - 1
        bigru_end_pos = int(row['Pred_End_Pos_BiGRU']) - 1
